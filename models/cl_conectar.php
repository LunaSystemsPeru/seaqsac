...
class cl_conectar
{
     ...
     */
    public function get_Cursor($query)
    {
        if (!self::es_string($query))
            exit();
        $db = cl_conectar::getInstancia();
        $mysqli = $db->getConnection();
        $resultado = $mysqli->query($query);
        return $resultado;
        // Este resultado se puede usar así:  while ($row = $resultado->fetch_assoc()){...}
        // a tambien asi foreach($array as $fila) { echo $fila['columna']; }
    }
    ...
    public function get_json_row($sql)
    {
        ...
        // Si hay un error en el SQL, este es el error de MySQL
        if (!$resultado) {
            print_r("class.cl_conectar.class: error " . $mysqli->error);
        }
        if (!$row = $resultado->fetch_assoc()) {
            return "{}";
        }
